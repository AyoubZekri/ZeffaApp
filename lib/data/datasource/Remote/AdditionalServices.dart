...
class AdditionalServices {
  ...
  Future<bool> Adddata(String name, double price) async {
    ...
    try {
      ...
      if (result > 0) {
        await _syncService.addToQueue("additional_services", uuid, "insert", data);
        return true;
      }
      return false;
    } catch (e) {
  ...
  Future<bool> UpdateData(String uuid, String name, double price) async {
    try {
      ...
      final result = await _db.update("additional_services", data, "uuid = ?", [uuid]);
      if (result > 0) {
  ...
  Future<bool> deletedata(String uuid) async {
    try {
      ...
      final result = await _db.delete("additional_services", "uuid = ?", [uuid]);
      if (result > 0) {
