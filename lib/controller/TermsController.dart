  ...
  Future<void> saveTerm() async {
    ...
    if (selectedType.value == 'required_documents') {
      final Map<String, dynamic> data = {
        ...
        "quantity": double.tryParse(singleMaterial.quantityController.text) ?? 1.0,
        "covers_guests": int.tryParse(singleMaterial.coversGuestsController.text) ?? 100,
      };
    ...
    if (term.type == 'required_documents') {
      if (term.contents.isNotEmpty) {
        try {
          ...
          singleMaterial.quantityController.text = decoded['quantity']?.toString().replaceAll(RegExp(r'\.0$'), '') ?? '1';
          singleMaterial.coversGuestsController.text = decoded['covers_guests']?.toString() ?? '100';
        } catch (e) {
          // Fallback
        }
      }
