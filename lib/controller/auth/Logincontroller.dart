      ...
      if (statusrequest == Statusrequest.success) {
        if (response["status"] == 1) {
          ...
          myServices.sharedPreferences!.setInt(
            "id",
            response['data']["user"]["user"]["parent"]['id'],
          );
          ...
          myServices.sharedPreferences!.setString(
            "numperPhone",
            response["data"]["user"]["user"]["parent"]["numperPhone"],
          );


          if (response["data"]["user"]["user"]["role_id"] == null) {
            Get.offAllNamed(Approutes.cannotUsePhone);
            return;
          }
          myServices.sharedPreferences!.setString(
            "type",
            response["data"]["user"]["user"]["role_details"]["type"],
          );
          myServices.sharedPreferences!.setString(
            "permissions",
            response["data"]["user"]["user"]["role_details"]["permissions"],
          );
          myServices.sharedPreferences!.setString(
            "role_name",
            response["data"]["user"]["user"]["role_details"]["name"],
          );
          if (response["data"]["user"]["user"]["parent"]["fieldPhone"] !=
              null) {
            myServices.sharedPreferences!.setString(
              "fieldPhone",
              response["data"]["user"]["user"]["parent"]["fieldPhone"],
            );
          }
          myServices.sharedPreferences!.setString(
            "hallname",
            response["data"]["user"]["user"]["parent"]["hallname"],
          );
          myServices.sharedPreferences!.setInt(
            "user_notify_status",
            response["data"]["user"]["user"]["user_notify_status"],
          );
          if (response["data"]["user"]["user"]["parent"]["adresse"] != null) {
            myServices.sharedPreferences!.setString(
              "adresse",
              response["data"]["user"]["user"]["parent"]["adresse"],
            );
          }

          if (response["data"]["user"]["user"]["parent"]["image"] != null) {
            myServices.sharedPreferences!.setString(
              "image",
              response["data"]["user"]["user"]["parent"]["image"],
            );
          }

          myServices.sharedPreferences!.setInt(
            "status",
            response["data"]["user"]["user"]["parent"]["status"],
          );
          if (response["data"]["user"]["user"]["parent"]["date_experiment"] !=
              null) {
            myServices.sharedPreferences!.setString(
              "date_experiment",
              response["data"]["user"]["user"]["parent"]["date_experiment"],
            );
          }
          myServices.sharedPreferences!.setString(
            "token",
          ...
          final status = response['data']["user"]["user"]["parent"]['status'];
          print("===========$status");

          if (status == 1) {
            Get.offAllNamed(Approutes.activateAccount);
            return;
          }
          if (status == 2) {
            Get.offAllNamed(Approutes.upgradeAccount);
            return;
          }
